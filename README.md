...
- `제작만`, `제작부터 1~5강`, `현재 강화 단계부터 목표 강화 단계` 계산
- 단계별 성공 확률, 최소 재료, 확률상 기대 재료 및 강화비 표시
- 제작 재료와 강화 재료를 분리해 표시하고 제작 재료 목록 복사 지원
...
- 공개 자료에서 0~5강 제작·강화 조합식을 확인한 장비 75종

여포의 관·방천화극, 최무선의 투구 및 만선야·레지나 장비는 공개 자료에서 0~5강 조합식이 완전하게 확인되지 않아 검색은 가능하지만 계산 결과는 `자료 확인 중`으로 표시합니다. 숫자를 임의 추정하지 않습니다.
